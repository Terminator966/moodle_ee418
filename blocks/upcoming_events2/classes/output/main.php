 ...
 */
namespace block_upcoming_events2\output;
...
class main implements renderable, templatable {
    ...
    public function __construct($order, $filter, $limit) {
        $this->order = $order ? $order : BLOCK_UPCOMING_EVENTS2_SORT_BY_DATES;
        $this->filter = $filter ? $filter : BLOCK_UPCOMING_EVENTS2_FILTER_BY_7_DAYS;
        $this->limit = $limit ? $limit : BLOCK_UPCOMING_EVENTS2_ACTIVITIES_LIMIT_DEFAULT;
    ...
    protected function get_filters_as_booleans() {
        $filters = [
            BLOCK_UPCOMING_EVENTS2_FILTER_BY_NONE => false,
            BLOCK_UPCOMING_EVENTS2_FILTER_BY_OVERDUE => false,
            BLOCK_UPCOMING_EVENTS2_FILTER_BY_7_DAYS => false,
            BLOCK_UPCOMING_EVENTS2_FILTER_BY_30_DAYS => false,
            BLOCK_UPCOMING_EVENTS2_FILTER_BY_3_MONTHS => false,
            BLOCK_UPCOMING_EVENTS2_FILTER_BY_6_MONTHS => false
        ];
    ...
    private function get_filter_offsets() {
        ...
        $limit = '';
        if (in_array($this->filter, [BLOCK_UPCOMING_EVENTS2_FILTER_BY_NONE, BLOCK_UPCOMING_EVENTS2_FILTER_BY_OVERDUE])) {
            $offset = -14;
            if ($this->filter == BLOCK_UPCOMING_EVENTS2_FILTER_BY_OVERDUE) {
                $limit = 1;
            }
        } else {
            $offset = 0;
            ...
            switch($this->filter) {
                case BLOCK_UPCOMING_EVENTS2_FILTER_BY_30_DAYS:
                    $limit = 30;
                    break;
                case BLOCK_UPCOMING_EVENTS2_FILTER_BY_3_MONTHS:
                    $limit = 90;
                    break;
                case BLOCK_UPCOMING_EVENTS2_FILTER_BY_6_MONTHS:
                    $limit = 180;
                    break;
            }
        }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $nocoursesurl = $output->image_url('courses', 'block_upcoming_events2')->out();
        $noeventsurl = $output->image_url('activities', 'block_upcoming_events2')->out();
        ...
        $contextvariables = [
            ...
            'sorttimelinedates' => $this->order == BLOCK_UPCOMING_EVENTS2_SORT_BY_DATES,
            'sorttimelinecourses' => $this->order == BLOCK_UPCOMING_EVENTS2_SORT_BY_COURSES,
