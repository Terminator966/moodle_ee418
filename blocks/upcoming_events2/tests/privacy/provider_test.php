...
/**
 * Unit tests for the block_upcoming_events2 implementation of the privacy API.
 *
...
use block_upcoming_events2\privacy\provider;
...
class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_preferences($type, $value, $expected) {
        ...
        provider::export_user_preferences($user->id);
        $writer = writer::with_context(\context_system::instance());
        $blockpreferences = $writer->get_user_preferences('block_upcoming_events2');
        if (!$expected) {
            $expected = get_string($value, 'block_upcoming_events2');
        }
    ...
    public function user_preference_provider() {
        return array(
            array('block_upcoming_events2_user_sort_preference', 'sortbydates', ''),
            array('block_upcoming_events2_user_sort_preference', 'sortbycourses', ''),
            array('block_upcoming_events2_user_sort_preference', 'next7days', ''),
            array('block_upcoming_events2_user_sort_preference', 'all', ''),
            array('block_upcoming_events2_user_limit_preference', 5, 5),
        );
    }
}
